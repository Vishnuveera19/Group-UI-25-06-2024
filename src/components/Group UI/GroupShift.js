...
import { Typography, Box, Checkbox, FormControl, InputLabel, Grid, TableContainer, Table, TableHead, TableBody, TableRow, TableCell, Paper, Button } from '@mui/material';
...
function GroupShift() {
    ...
    const handleCheckboxChange = (employeeCode, isChecked) => {
        if (isChecked) {
            ...
            setCheckedRows(prev => prev.filter(row => row.EmployeeCode !== employeeCode));
    ...
    const handlesave = async () => {
        if (checkedRows.length > 0) {
            const values = checkedRows.map(row => `('${row.EmployeeCode}', '${row.GroupID}')`).join(', ');
            ...
            <FormControl fullWidth>
                                            ...
                                            >
                                                ...
                                                {grpSettings.map((group) => (
                                                    <option key={group.GroupID} value={group.GroupID}>{group.Group_name}</option>
                                                ))}
                                            </select>
                                        </FormControl>
                                        </Grid>
                ...
                <Table>
                    <TableHead>
                        <TableRow>
                            ...
                            <TableCell style={{fontSize:'16px'}}>Employee Full Name</TableCell>
