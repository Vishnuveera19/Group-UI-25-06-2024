...
import { putRequest } from "../../serverconfiguration/requestcomp";
...
const JsonTable = ({ jsonData, url }) => {
  ...
  const handleSave = (rowIndex, row) => {
    ...
    //   })
    console.log(row);
    putRequest(ServerConfig.url, EARNDEDUCT + "/" + row.pnCompanyId, row)
      .then((response) => {
      ...
      <div style={{ maxWidth: "100%", marginLeft: "570px" }}>
        <div
          ...
          <select
            ...
            {jsonData.length==0 ||jsonData==undefined?"No data available ": Object.keys(jsonData[0]).map((header) => (
              <option>
                {header == "pnCompanyid" ||
                header == "pnBranchid" ||
                header == "empCode" ||
