...
import { PAYMEMPLOYEE, TIMECARD, UPDATEDTIMECARD, SHIFTMONTH, REPORTS,PAYMEMPLOYEEPROFILE1 } from '../../serverconfiguration/controllers';
...
export default function AttendanceNew() {
  ...
  const [employeeProfile1,setEmployeeprofile1] = useState([]);
  ...
  useEffect(() => {
    async function getData() {
      ...
      const employeeProfile1 = await getRequest(ServerConfig.url,PAYMEMPLOYEEPROFILE1);
      setEmployeeprofile1(employeeProfile1.data);
      const groupsettings = await postRequest(ServerConfig.url,REPORTS, {
        "query" : `select * from Group_Settings`
      })
      setgroupsettings(groupsettings.data);
      console.log(groupsettings)
      return timecard.data
      const currentdata=await postRequest(ServerConfig.url,REPORTS,{
                  ...
                  <option value="">Select</option>
                     {
                          ...
                          employeeProfile1.filter((e)=>(e.pnCompanyId==company && e.pnBranchId == branch)).map((e)=><option key={e.Id}>{e.pnDepartmentId}</option>)
                     }
                 </select>
                 </FormControl>
                  </Grid>
...
"
                      ...
                      style={{ height: '50px' }}>
                      <option value="">Select</option>
                      {groupsettings.map((e) => <option >{e.groupid}</option>)}
                    </select>
                  </FormControl>
                </Grid>
